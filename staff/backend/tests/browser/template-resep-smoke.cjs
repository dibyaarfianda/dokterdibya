...
(async()=>{ const server=await new Promise(resolve=>{const s=app.listen(0,'127.0.0.1',()=>resolve(s));}); const browser=await puppeteer.launch({headless:true,executablePath:process.env.CHROME_PATH||'C:/Program Files/Google/Chrome/Application/chrome.exe'}); try {
 ...
 await page.addScriptTag({url:'/staff/public/scripts/shell/actions.js'});
 await page.evaluate(()=>{window.staffRoleConstants={ROLE_IDS:{DOKTER:1}};openMobileMenu();});assert.ok(await page.$eval('#dynamic-mobile-menu',n=>n.textContent.includes('Template Resep')),'Mobile menu must expose templates');await page.evaluate(()=>closeMobileMenu());
 ...
 await page.$eval('#rx-name',n=>{n.value='Template Diedit';n.dispatchEvent(new Event('input',{bubbles:true}));});await page.click('#bell');assert.equal(await page.$eval('#rx-editor',n=>n.hidden),false,'Notification toggle must retain dirty editor');accept=false;await page.click('#leave');assert.match(page.url(),/qa#?$/);await page.evaluate(()=>{const b=document.createElement('button');b.id='mobile-test';b.dataset.shellAction='mobile-nav';b.dataset.mobileNav='dashboard';document.body.appendChild(b);window.showDashboardPage=()=>{window.didLeaveEditor=true;};});await page.click('#mobile-test');assert.equal(await page.evaluate(()=>Boolean(window.didLeaveEditor)),false,'Cancelled bottom navigation must retain editor');accept=true;
