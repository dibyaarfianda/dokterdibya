...
services.push({id:1,name:'Biaya Admin',price:15000,is_active:1},{id:3,name:'Buku Kontrol Obstetri',price:25000,is_active:1},{id:59,name:'Buku Kontrol Ginekologi',price:25000,is_active:1});
...
(async () => {
    ...
    try {
        ...
        assert.equal(await frame.$eval('[data-estimate="repeat"]', n => n.value), '0');
        ...
        assert.ok(await frame.$$eval('[data-estimate="service"]', ns => ns.length > 0 && ns.every(n => n.value === '0')));
        assert.match(await frame.$eval('.estimate-summary', n => n.textContent), /Subtotal layananRp 0/);
        await frame.select('[data-estimate="trimester"]', 'all');
        assert.ok(await frame.$$eval('input[type="number"]', ns => ns.every(n => n.value === '0')), 'all trimester controls start at zero');
        assert.equal(await frame.$eval('#estimate-total', n => n.textContent), 'Rp 0');
        for (const key of await frame.$$eval('[data-estimate="service"]', ns => ns.map(n => n.dataset.key))) await change('[data-estimate="service"][data-key="'+key+'"]', '');
        for (const key of ['t1','t2','t3']) {
            await change('[data-estimate="visits"][data-key="'+key+'"]', '');
            await change('[data-estimate="repeat"][data-key="'+key+'"]', '1');
        }
        ...
        for (const key of ['t1','t2','t3']) await change('[data-estimate="visits"][data-key="'+key+'"]', '1');
        assert.equal(await frame.$eval('#estimate-total', n => n.textContent), 'Rp 865.000');
        ...
        await change('[data-estimate="visits"]', '');
        await change('[data-estimate="repeat"]', '1');
        assert.equal(await frame.$eval('#estimate-total', n => n.textContent), 'Belum dihitung');
        await change('[data-estimate="service"]', '1');
        assert.equal(await frame.$eval('#estimate-total', n => n.textContent), 'Belum dihitung');
        assert.equal(await frame.$eval('[data-estimate="visits"]', n => n.value), '');
        await change('[data-estimate="visits"]', '1');
        assert.equal(await frame.$eval('#estimate-total', n => n.textContent), 'Rp 185.000');
        await frame.select('[data-estimate="book"]', 'ginekologi');
        assert.equal(await frame.$eval('#estimate-total', n => n.textContent), 'Rp 185.000');
        await change('[data-estimate="visits"]', '');
        assert.equal(await frame.$eval('#estimate-total', n => n.textContent), 'Belum dihitung');
        await change('[data-estimate="visits"]', '1');
        ...
        assert.equal(await frame.$eval('#estimate-total', n => n.textContent), 'Rp 230.000');
        await change('[data-estimate="repeat"]', '');
        assert.equal(await frame.$eval('#estimate-total', n => n.textContent), 'Belum dihitung');
        assert.match(await frame.$eval('.estimate-repeat-hint', n => n.textContent), /Acuan: 1 kali/);
        await change('[data-estimate="repeat"]', '0');
        assert.equal(await frame.$eval('#estimate-total', n => n.textContent), 'Rp 140.000');
        await change('[data-estimate="service"]', '0');
        assert.equal(await frame.$eval('#estimate-total', n => n.textContent), 'Rp 40.000');
        await change('[data-estimate="visits"]', '0');
        assert.equal(await frame.$eval('#estimate-total', n => n.textContent), 'Rp 0');
        ...
        await frame.select('[data-estimate="trimester"]', 't1');
        assert.equal(await frame.$eval('[data-estimate="service"]', n => n.value), '0', 'refresh resets service simulation');
        await change('[data-estimate="service"]', '1');
