...
describe('patient Google registration flow contract', () => {
    ...
    test('patient web login uses registration-first Google flow only', () => {
        ...
        expect(rootSw).toContain("const CACHE_VERSION = '20260622storyspace1';");
        expect(sisiwanitaSw).toContain("const CACHE_VERSION = '20260622storyspace1';");
        expect(patientMenu).toContain('/patient-portal.webmanifest?v=20260622storyspace1');
        expect(patientMenu).toContain("/sw.js?v=20260622storyspace1");
        expect(manifest).toContain('/patient-menu.html?source=pwa&v=20260622storyspace1');
    });
});
