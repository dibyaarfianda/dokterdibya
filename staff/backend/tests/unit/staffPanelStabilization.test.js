...
describe('staff panel stabilization sources', () => {
    test('uses one v262 cache version source for staff assets', () => {
        const html = readRepoFile('staff', 'public', 'index-adminlte.html');

        expect(html).toContain("window.STAFF_CACHE_VERSION = 'v262';");
        ...
        expect(html).toContain('styles/mobile-responsive.css?v=v262');
        expect(html).not.toMatch(/CACHE_VERSION\s*=\s*'v241'/);
        expect(html).not.toMatch(/__assetVersion\s*=\s*'v250'/);
    });

    test('service worker v262 precache does not include missing chat panel css', () => {
        const sw = readRepoFile('staff', 'public', 'sw.js');

        expect(sw).toContain("const STAFF_PWA_VERSION = 'v262';");
        expect(sw).not.toContain('/staff/public/styles/chat-slide-panel.css');
    });

    test('staff PWA mobile typography uses Compact 11 with explicit allowlisted exceptions', () => {
        ...
        expect(mobileCss).toContain('Compact 11 text scale with component-specific icon exceptions.');
        ...
        expect(sundayClinicCss).toContain('Compact 11 final pass for Sunday Clinic PWA/mobile.');
    ...
    test('staff mobile tap feedback uses COMM tap sound parameters', () => {
        ...
        expect(html).toContain('/staff/public/scripts/tap-feedback.js?v=v262');
        expect(sundayClinicHtml).toContain('/staff/public/scripts/tap-feedback.js?v=v262');
