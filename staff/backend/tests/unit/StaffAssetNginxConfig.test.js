...
describe('Staff Nginx release routing', () => {
    test('disposable Nginx browser probe uses system Chrome with sandbox and loopback routing', () => {
        const workflow = yaml.load(fs.readFileSync(path.resolve(__dirname, '../../../../.github/workflows/staff-panel-ci.yml'), 'utf8'));
        const job = workflow.jobs['staff-asset-nginx'];
        ...
        const installIndex = steps.findIndex(step => step.run?.includes('apt-get install -y nginx-light openssl'));
        const chromeIndex = steps.findIndex(step => step.run?.includes('test -x "$PUPPETEER_EXECUTABLE_PATH"'));
        expect(installIndex).toBeGreaterThan(-1);
        expect(chromeIndex).toBeGreaterThan(installIndex);
        expect(steps[chromeIndex].run).toContain('"$PUPPETEER_EXECUTABLE_PATH" --version');
        expect(steps[chromeIndex].run).toContain('puppeteer.launch({ headless: true })');
        expect(steps.every(step => !step.run?.includes('browsers install chrome'))).toBe(true);
        const probeIndex = steps.findIndex(step => step.run?.includes('--probe-nginx'));
        expect(probeIndex).toBeGreaterThan(chromeIndex);
        expect(steps[probeIndex].run).toMatch(/^node /);
        expect(steps.find(step => step.run?.includes('nginx -s quit')).if).toBe('always()');
        const browserProbe = fs.readFileSync(path.resolve(__dirname, './StaffAssetModulePropagation.test.js'), 'utf8');
        expect(browserProbe).not.toContain('--no-sandbox');
        expect(browserProbe).toContain('--no-proxy-server');
        expect(browserProbe).toContain('--host-resolver-rules=MAP dokterdibya.com 127.0.0.1');
    });
