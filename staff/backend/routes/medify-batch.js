...
/**
 * POST /api/medify-batch/reverse-sync/:source
 * Scan SIMRS by date range and match against ALL registered patients
 * This allows detecting patients who switched locations or are newly registered
 */
router.post('/reverse-sync/:source', verifyToken, requireDocterOrAdmin, async (req, res) => {
    const { source } = req.params;
    ...
    try {
        ...
        console.log(`[ReverseSync] Starting for ${source}, dates: ${dateStart} to ${dateEnd}`);
        ...
        // Search patient history by date
        const simrsPatients = await medifyService.searchPatientHistory(page, source, dateStart, dateEnd);
        console.log(`[ReverseSync] Found ${simrsPatients.length} patients in SIMRS`);

        if (simrsPatients.length === 0) {
            await page.close();
            return res.json({
                success: true,
                message: 'No patients found in SIMRS for the given date range',
                results: {
                    matched_with_drd: [],
                    matched_no_drd: [],
                    partial_match: [],
        ...
        const patientDrdMap = new Map();
        existingDrds.forEach(r => {
            if (!patientDrdMap.has(r.patient_id)) {
                patientDrdMap.set(r.patient_id, r.mr_id);
            }
        });

        // Process each SIMRS patient
        const results = {
            matched_with_drd: [],
            matched_no_drd: [],
            partial_match: [],
        ...
        for (const simrsPatient of simrsPatients) {
            try {
                // Extract identity for this patient
                const identity = await medifyService.extractPatientIdentity(page, source, simrsPatient.medId);
                await medifyService.delay(500); // Rate limiting

                // Merge basic info with identity
                const simrsData = {
                    ...simrsPatient,
                    nama: identity.nama || simrsPatient.name,
                    tanggal_lahir: identity.tanggal_lahir,
                    usia: identity.usia,
                    alamat: identity.alamat,
                    no_hp: identity.no_hp
                };

                // Find best match among ALL registered patients
                let bestMatch = null;
                let bestMatchCount = 0;
                let bestFactors = [];

                for (const dibyaPatient of allPatients) {
                    const matchResult = medifyService.countMatchingFactors(simrsData, {
                        full_name: dibyaPatient.full_name,
                    ...
                    if (matchResult.matchCount > bestMatchCount) {
                        ...
                        bestMatch = dibyaPatient;
                        bestFactors = matchResult.factors;
                    }
                }

                const resultItem = {
                    simrs: {
                        name: simrsData.nama || simrsPatient.name,
                        medId: simrsPatient.medId,
                        visitDate: simrsPatient.visitDate,
                        dob: simrsData.tanggal_lahir,
                        age: simrsData.usia,
                        phone: simrsData.no_hp,
                        address: simrsData.alamat
                    },
                    matchCount: bestMatchCount,
                    factors: bestFactors
                };

                if (bestMatchCount >= 3 && bestMatch) {
                    // Good match - check if has DRD at this location
                    const existingDrd = patientDrdMap.get(bestMatch.id);
                    resultItem.patient = {
                        id: bestMatch.id,
                        name: bestMatch.full_name,
                        dob: bestMatch.birth_date,
                        phone: bestMatch.whatsapp || bestMatch.phone
                    };

                    if (existingDrd) {
                        resultItem.drd = existingDrd;
                        results.matched_with_drd.push(resultItem);
                    } else {
                        results.matched_no_drd.push(resultItem);
                    }
                } else if (bestMatchCount === 2 && bestMatch) {
                    ...
                    results.partial_match.push(resultItem);
                } else {
                    // No match - likely new patient
                    results.no_match.push(resultItem);
                }

            } catch (err) {
                console.error(`[ReverseSync] Error processing ${simrsPatient.name}:`, err.message);
                results.no_match.push({
                    simrs: {
                        name: simrsPatient.name,
                        medId: simrsPatient.medId,
                        visitDate: simrsPatient.visitDate
                    },
                    matchCount: 0,
        ...
        // Log activity
        await activityLogger.log(userId, userName, 'MEDIFY Reverse Sync',
            `Source: ${source}, Date: ${dateStart}-${dateEnd}, Found: ${simrsPatients.length}, ` +
            `Matched: ${results.matched_with_drd.length + results.matched_no_drd.length}, ` +
            `Partial: ${results.partial_match.length}, New: ${results.no_match.length}`);
        ...
        res.json({
            ...
            summary: {
                ...
                matched_with_drd: results.matched_with_drd.length,
                matched_no_drd: results.matched_no_drd.length,
                partial_match: results.partial_match.length,
...
/**
 * POST /api/medify-batch/reverse-sync/:source/create-drd
 * Create DRD for matched patient at new location
 */
router.post('/reverse-sync/:source/create-drd', verifyToken, requireDocterOrAdmin, async (req, res) => {
    const { source } = req.params;
    const { patientId, simrsMedId, category } = req.body;
    const userId = req.user.id;
    const userName = req.user.name;

    try {
        // Validate
        if (!patientId || !simrsMedId) {
            return res.status(400).json({
                success: false,
                message: 'patientId and simrsMedId are required'
            });
        }
        ...
        if (existing && existing.length > 0) {
            return res.json({
                success: true,
                message: 'Patient already has DRD at this location',
                mrId: existing[0].mr_id,
                alreadyExists: true
            });
        }
        ...
        try {
            ...
            const [seqResult] = await conn.query(
                `SELECT MAX(mr_sequence) as maxSeq FROM sunday_clinic_records FOR UPDATE`
            );
            const nextSeq = (seqResult[0].maxSeq || 0) + 1;
            const mrId = `DRD${String(nextSeq).padStart(4, '0')}`;

            // Create sunday_clinic_record
            await conn.query(
                `INSERT INTO sunday_clinic_records
                 (mr_id, mr_sequence, patient_id, visit_location, mr_category, status, created_by)
                 VALUES (?, ?, ?, ?, ?, 'waiting', ?)`,
                [mrId, nextSeq, patientId, source, category || 'obstetri', userId]
            );

            await conn.commit();

            // Log activity
            await activityLogger.log(userId, userName, 'Create DRD (Reverse Sync)',
                `Patient: ${patient.full_name}, MR ID: ${mrId}, Location: ${source}`);

            res.json({
                success: true,
                message: 'DRD created successfully',
                mrId,
                patientId,
        ...
        console.error('[ReverseSync] Error creating DRD:', error);
        res.status(500).json({ success: false, message: error.message });
    }
});

/**
 * POST /api/medify-batch/reverse-sync/:source/import
 * Import medical record from SIMRS for a specific patient
 */
router.post('/reverse-sync/:source/import', verifyToken, requireDocterOrAdmin, async (req, res) => {
    const { source } = req.params;
    const { mrId, simrsMedId } = req.body;
    const userId = req.user.id;
    const userName = req.user.name;
    ...
    try {
        if (!mrId || !simrsMedId) {
            return res.status(400).json({
                success: false,
                message: 'mrId and simrsMedId are required'
            });
        }

        // Get browser and extract CPPT
        const browser = await medifyService.getBrowser();
        const page = await browser.newPage();
        await page.setViewport({ width: 1920, height: 1080 });

        await medifyService.login(page, source);
        const cpptResult = await medifyService.extractCPPT(page, source, simrsMedId);
        await page.close();

        if (!cpptResult || !cpptResult.success) {
            return res.status(400).json({
                success: false,
                message: 'Failed to extract CPPT from SIMRS'
            });
        }
        ...
        for (const [sectionType, data] of Object.entries(sections)) {
            if (data && Object.keys(data).length > 0) {
                await pool.query(
                    `INSERT INTO sunday_clinic_medical_records
                     (record_id, section_type, record_data, created_by)
                     VALUES (?, ?, ?, ?)
        ...
        // Log activity
        await activityLogger.log(userId, userName, 'MEDIFY Import (Reverse Sync)',
            `MR ID: ${mrId}, SIMRS Med ID: ${simrsMedId}, Source: ${source}`);

        res.json({
            success: true,
            message: 'Medical record imported successfully',
            mrId,
            sectionsImported: Object.keys(sections).length
        });

    } catch (error) {
        console.error('[ReverseSync] Error importing:', error);
        res.status(500).json({ success: false, message: error.message });
    }
});
