...
// Called only from MedicalRecordService.mutateDocuments while its visit and
// section locks are held. Notification and patient refresh follow commit.
async function mutatePenunjangDocuments(connection, { patientId, mrId, files = [], actorUserId = null }) {
    const normalizedFiles = Array.isArray(files) ? files.filter(file => file && file.url) : [];
    const [existingDocs] = await connection.query(
        `SELECT id, file_url FROM patient_documents
         ...
         ORDER BY id FOR UPDATE`, [patientId, mrId]);
    ...
    if (toDelete.length) await connection.query('DELETE FROM patient_documents WHERE id IN (?)', [toDelete.map(doc => doc.id)]);
    ...
    for (const file of toInsert) {
        await connection.query(
            ...
            [patientId, mrId, file.name || 'Hasil Lab', file.url, file.key || file.filename || null,
                file.name || 'Hasil Lab', file.type || 'application/octet-stream', file.size || 0,
