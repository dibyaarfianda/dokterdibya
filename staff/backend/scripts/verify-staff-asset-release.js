...
function parseArguments(args) {
    ...
    if (args.length < 12 || args.length % 2) throw new Error('Staff verification requires origin, release base, current version, versions and paths');
    for (let index = 0; index < args.length; index += 2) {
        ...
        else if (flag === '--expected-current-version' && !options.expectedCurrentVersion) options.expectedCurrentVersion = value;
    ...
    if (!options.baseUrl || !options.releaseBase || !options.expectedCurrentVersion ||
        options.versions.length < 2 || !options.paths.length) {
