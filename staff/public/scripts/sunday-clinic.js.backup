    ...
    switch (activeSection) {
        ...
        case 'anamnesa':
            ...
            if (manualCategory) {
                ...
                // Determine if patient is obstetric based on pregnant_status ONLY
                // Do NOT use LMP/EDD as fallback - these fields exist for non-obstetric patients too
                if (pregnantStatus === 'yes') {
                    isObstetric = true;
                } else {
                    // Default to non-obstetric (gynecology) if not explicitly marked as pregnant
                    // This prevents anobs/anpro patients from being wrongly classified as obstetric
                    isObstetric = false;
                    console.log('No pregnant_status=yes, defaulting to gynecology (non-obstetric)');
                }
            }
