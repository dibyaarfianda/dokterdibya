...
(function() {
    ...
    function extractCPPT() {
        ...
        let cpptText = '';
        let fullText = '';  // Keep full text including signature for date extraction
        ...
        console.log('[SIMRS Export] Attempting date extraction from text length:', fullText.length);
        const signatureMatch1 = fullText.match(/(\d{1,2})\s+(January|February|March|April|May|June|July|August|September|October|November|December)\s+(\d{4})\s+(\d{1,2}:\d{2})/i);
        console.log('[SIMRS Export] Pattern 1 (English) match result:', signatureMatch1);
        if (signatureMatch1) {
            ...
            console.log('[SIMRS Export] Extracted signature date/time:', cpptData.visit_date, cpptData.visit_time);
        } else {
            // Pattern 2: "30 Desember 2025 21:20" (Indonesian month with time)
            const signatureMatch2 = fullText.match(/(\d{1,2})\s+(Januari|Februari|Maret|April|Mei|Juni|Juli|Agustus|September|Oktober|November|Desember)\s+(\d{4})\s+(\d{1,2}:\d{2})/i);
            if (signatureMatch2) {
                ...
                console.log('[SIMRS Export] Extracted signature date/time (ID):', cpptData.visit_date, cpptData.visit_time);
            } else {
                // Pattern 3: "30/12/2025 10:30" or "30-12-2025 10:30"
                const signatureMatch3 = fullText.match(/(\d{1,2})[-\/](\d{1,2})[-\/](\d{4})\s+(\d{1,2}:\d{2})/);
                console.log('[SIMRS Export] Pattern 3 (numeric) match result:', signatureMatch3);
                if (signatureMatch3) {
                    ...
                    console.log('[SIMRS Export] Extracted numeric date/time:', cpptData.visit_date, cpptData.visit_time);
                } else {
                    console.warn('[SIMRS Export] NO DATE PATTERN MATCHED! Check text for date format.');
                }
            }
        }
        ...
        try {
            ...
            if (result.success) {
                ...
                if (localCPPT.visit_date) {
                    ...
                    console.log('[RSIA Melinda] Using local visit_date:', localCPPT.visit_date);
                }
                if (localCPPT.visit_time) {
                    mergedData.visit_time = localCPPT.visit_time;
                    console.log('[RSIA Melinda] Using local visit_time:', localCPPT.visit_time);
                }
