(function () {
    ...
    const initialScenario = () => ({ trimester: 't1', repeats: Object.fromEntries(TRIMESTERS.map(key => [key, null])), services: {} });
    function create(container, shell) {
        ...
        function render() {
            ...
            const pending = TRIMESTERS.some(key => (scenario.trimester === 'all' || scenario.trimester === key) && missingRepeat(key));
            container.innerHTML = '<div class="estimate-banner">' + (data.is_dummy ? 'Data Dummy — bukan tarif klinik' : 'Pratinjau pasien — belum diterbitkan') + '</div>' +
                ...
                TRIMESTERS.filter(key => scenario.trimester === 'all' || key === scenario.trimester).map(key => {
                    ...
                    return '<section class="section estimate-phase"><div class="tool-panel"><div class="estimate-heading"><h3>Trimester ' + (TRIMESTERS.indexOf(key) + 1) + '</h3>' +
                        ...
                        phase.items.map(item => '<div class="estimate-item"><div class="estimate-item-top"><strong>' + esc(item.label) + '</strong><strong class="estimate-price">' + money(item.subtotal, item.kind === 'medication' && missingRepeat(key)) + '</strong></div>' +
                        ...
                        '<div class="estimate-summary"><div><span>Subtotal obat / suplemen</span><strong>' + money(phase.medication_total, missingRepeat(key)) + '</strong></div><div><span>Subtotal layanan</span><strong>' + money(phase.service_total) + '</strong></div><div class="estimate-phase-total"><span>Total trimester</span><strong>' + money(phase.total, missingRepeat(key)) + '</strong></div></div></div></section>';
                }).join('') +
                (!result.ready ? '<div class="estimate-warning">' + (pending ? 'Isi jumlah pengulangan resep pada periode yang dipilih untuk menghitung estimasi. Angka acuan mengikuti pengaturan klinik.' : 'Total belum tersedia. Lengkapi konfigurasi atau periksa angka pengulangan pada periode yang dipilih.') + '</div>' : '') +
        ...
        function help() {
            ...
            shell.openModal('Cara Menggunakan', 'Estimasi biaya', '<div class="estimate-help"><ol>' +
                ...
                '<li><strong>Atur pengulangan.</strong><br>Isi kolom pengulangan resep yang masih kosong; lihat angka acuan di sampingnya. Total dihitung setelah angka diisi. Resep dan setiap layanan memiliki pengulangan terpisah. Isi 0 untuk tidak memasukkannya dalam simulasi.</li>' +
        ...
        return {
            setData(value) {
                data = value; scenario = initialScenario(); render();
