            ...
            container.innerHTML = '<div class="estimate-banner">' + (data.is_dummy ? 'Data Dummy — bukan tarif klinik' : data.is_published ? 'Estimasi biaya kontrol kehamilan' : 'Pratinjau pasien — belum diterbitkan') + '</div>' +
                ...
                selectedKeys.map(key => {
                    ...
                    return '<section class="section estimate-phase"><div class="tool-panel"><div class="estimate-heading"><h3>Trimester ' + (TRIMESTERS.indexOf(key) + 1) + '</h3>' +
                        ...
                        phase.items.map(item => '<div class="estimate-item"><div class="estimate-item-top"><strong>' + esc(item.label) + '</strong></div>' +
                            '<div class="estimate-item-detail"><span>' + (item.kind === 'medication' ? 'Obat / suplemen' : 'Layanan / pemeriksaan') + '<br>' +
                            esc(item.quantity) + ' ' + esc(item.unit) + ' · ' + esc(item.repeats ?? '…') + ' pengulangan</span>' +
